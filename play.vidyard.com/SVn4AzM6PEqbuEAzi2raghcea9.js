var vidyard_player_width_SVn4AzM6PEqbuEAzi2ragh = 640;
var vidyard_player_height_SVn4AzM6PEqbuEAzi2ragh = 360;
var vidyard_html5_SVn4AzM6PEqbuEAzi2ragh = false;
var vidyard_secure_SVn4AzM6PEqbuEAzi2ragh = false;
var vidyard_integration_check_SVn4AzM6PEqbuEAzi2ragh = function() {};
var vidyard_SVn4AzM6PEqbuEAzi2ragh_params = {};
var vidyard_SVn4AzM6PEqbuEAzi2ragh_raw_params = '';
...
(function() {
  ...
  var embed_script = document.getElementById('vidyard_embed_code_SVn4AzM6PEqbuEAzi2ragh');
  var playerDetails = {"keywords":"ActivPanel,Bett 2018,ClassFlow,Immersive Learning,Interactive Displays,Interactive Software","thumbnailUrl":"https://play.vidyard.com/SVn4AzM6PEqbuEAzi2ragh.jpg","@context":"http://schema.org/","@type":"VideoObject","id":"SVn4AzM6PEqbuEAzi2ragh","description":"Experience Immersive Learning with Promethean solutions from interactive displays to interactive software.","name":"Immersive Learning","transcript":"","uploadDate":"2018-02-12T19:58:05.000Z","duration":"T1M38S","embedUrl":"https://play.vidyard.com/SVn4AzM6PEqbuEAzi2ragh","height":360,"width":640};
  ...
  function check_for_duplicate_embeds() {
    ...
    for (var i = 0; i < all_scripts.length; ++i) {
      if (all_scripts[i].getAttribute('id') === "vidyard_embed_code_SVn4AzM6PEqbuEAzi2ragh") {
    ...
    if (dupe_scripts.length >= 1) {
      ...
      if (typeof document.querySelectorAll === 'function') {
        ...
        for (var i = 0; i < wrappers.length; ++i) {
          if (wrappers[i].getAttributeNode('onclick').value === "fn_vidyard_SVn4AzM6PEqbuEAzi2ragh();") {
  ...
  if (!embed_script || !embed_script.src) {
    ...
    for (var i in scripts) {
      ...
      if (script.src && /^.+(vidyard\.com).*\/SVn4AzM6PEqbuEAzi2ragh.*/.test(script.src)) {
        embed_script = script;
        break;
      }
    }
  ...
  if (embed_script && embed_script.src) {
    ...
    vidyard_SVn4AzM6PEqbuEAzi2ragh_raw_params = rawQueryParams;
    ...
    embed_script.src.replace(/(?:[\?&])([^&=]*)=?([^&#]*)/g, function ($0, $1, $2) {
      if ($1) {
        switch ($1) {
          ...
          case 'vylegacy':
            vidyard_SVn4AzM6PEqbuEAzi2ragh_params[$1] = (parseInt($2, 10) !== 0);
            break;
          case 'chapter':
            vidyard_SVn4AzM6PEqbuEAzi2ragh_params[$1] = parseInt($2, 10);
            break;
          case 'width':
          ...
          case 'height':
            ...
            break;
          case 'X-VY-Signature':
            vidyard_secure_SVn4AzM6PEqbuEAzi2ragh = true;
          default:
            vidyard_SVn4AzM6PEqbuEAzi2ragh_params[$1] = $2;
            break;
        }
      }
    });
  ...
  if (!vidyard_SVn4AzM6PEqbuEAzi2ragh_params.type || vidyard_SVn4AzM6PEqbuEAzi2ragh_params.type !== 'lightbox') {
    vidyard_SVn4AzM6PEqbuEAzi2ragh_params.type = 'inline';
  }

  // Determine Player Settings which effect embed
  var multi_chapter = false && !(typeof vidyard_SVn4AzM6PEqbuEAzi2ragh_params.hide_html5_playlist !== 'undefined' && vidyard_SVn4AzM6PEqbuEAzi2ragh_params.hide_html5_playlist);
  var playlist_always_open = false;
  if (typeof vidyard_SVn4AzM6PEqbuEAzi2ragh_params.playlist_always_open !== 'undefined') {
    playlist_always_open = vidyard_SVn4AzM6PEqbuEAzi2ragh_params.playlist_always_open;
  }
  ...
  if (query_player_width && query_player_width > 0 && query_player_height && query_player_height > 0) {
    vidyard_player_width_SVn4AzM6PEqbuEAzi2ragh = query_player_width;
    vidyard_player_height_SVn4AzM6PEqbuEAzi2ragh = query_player_height;
  } else if (query_player_width && query_player_width > 0) {
    query_player_height = (query_player_width * vidyard_player_height_SVn4AzM6PEqbuEAzi2ragh) / vidyard_player_width_SVn4AzM6PEqbuEAzi2ragh;
    vidyard_player_width_SVn4AzM6PEqbuEAzi2ragh = query_player_width;
    vidyard_player_height_SVn4AzM6PEqbuEAzi2ragh = query_player_height;
  } else if (query_player_height && query_player_height > 0) {
    query_player_width = (query_player_height * vidyard_player_width_SVn4AzM6PEqbuEAzi2ragh) / vidyard_player_height_SVn4AzM6PEqbuEAzi2ragh;
    vidyard_player_width_SVn4AzM6PEqbuEAzi2ragh = query_player_width;
    vidyard_player_height_SVn4AzM6PEqbuEAzi2ragh = query_player_height;
  }

  var iframe_container = document.getElementById("vidyard_SVn4AzM6PEqbuEAzi2ragh");
  ...
  function fn_getCookie(cookie_name) {
    ...
    var ARRcookies = document.cookie.split(";");
      ...
      if(cookie_name === 'pardot'){
        cookie_name = "visitor_id18462";
      }
  ...
  if (!iframe_container && vidyard_SVn4AzM6PEqbuEAzi2ragh_params.type === 'inline') {
    iframe_container = document.createElement("span");
    iframe_container.id = 'vidyard_SVn4AzM6PEqbuEAzi2ragh';
  ...
  if (vidyard_secure_SVn4AzM6PEqbuEAzi2ragh === false) {
    // Get cookie data/ids from various MAPs/CRMs
    var pardot_id = fn_getCookie("pardot");
    if (pardot_id) {
      vidyard_SVn4AzM6PEqbuEAzi2ragh_params.pardot_id = pardot_id;
    }

    var hubspot_id = fn_getCookie("hubspotutk");
    if (hubspot_id) {
      vidyard_SVn4AzM6PEqbuEAzi2ragh_params.hubspot_id = hubspot_id;
    }
    ...
    if (marketo_id && munchkin_id && marketo_id.toLowerCase().indexOf(munchkin_id) > -1) {
      vidyard_SVn4AzM6PEqbuEAzi2ragh_params.marketo_id = encodeURIComponent(marketo_id);
    }

    var dreamforce_id = fn_getCookie("vy_dreamforce");
    if (dreamforce_id) {
      vidyard_SVn4AzM6PEqbuEAzi2ragh_params.dreamforce_id = encodeURIComponent(dreamforce_id);
    }
    ...
    if (vycustom_id !== "") {
      vidyard_SVn4AzM6PEqbuEAzi2ragh_params.custom_id = vycustom_id;
    }

    // Gets the vyemail query string param, which connects views to an email entered
    var vyemail = get_parameter_by_name('vyemail');
    if (vyemail !== "") {
      vidyard_SVn4AzM6PEqbuEAzi2ragh_params.email = vyemail;
    }

    var vyac = get_parameter_by_name('vyac');
    var vyplayer = get_parameter_by_name('vyplayer');
    if (vyplayer === 'SVn4AzM6PEqbuEAzi2ragh') {
      vidyard_SVn4AzM6PEqbuEAzi2ragh_params.access_code = vyac;
    }
  }
  ...
  if (navigator.userAgent.toLowerCase().match(/ip(ad|od|hone)|android/)) {
    vidyard_html5_SVn4AzM6PEqbuEAzi2ragh = true;
    vidyard_SVn4AzM6PEqbuEAzi2ragh_params.disable_popouts = true;
  }
  ...
  function create_embed() {
    ...
    var integration_tracking_check = function() {
      if (playerReady) {
        ...
        window.addEventListener('message', function callback(e) {
          ...
          if (data.event === "ready" && data.uuid === "SVn4AzM6PEqbuEAzi2ragh") {
    ...
    var integration_tracking_check_original = function () {
      ...
      if (!referringUrlSent && vidyard_secure_SVn4AzM6PEqbuEAzi2ragh === true && vidyard_SVn4AzM6PEqbuEAzi2ragh_params.referring_url) {
        var playerFrame = Vidyard.Helpers.getPlayerIFrame('SVn4AzM6PEqbuEAzi2ragh');
        var message = {
          'event': 'parentReferer',
          'uuid': 'SVn4AzM6PEqbuEAzi2ragh',
          'data': {
            'value': vidyard_SVn4AzM6PEqbuEAzi2ragh_params.referring_url
          }
        }
      ...
      if (!vidyard_SVn4AzM6PEqbuEAzi2ragh_params.pardot_id) {
        var pardot_id = fn_getCookie("pardot");
        if (pardot_id) {
          vidyard_SVn4AzM6PEqbuEAzi2ragh_params.pardot_id = pardot_id;
      ...
      if (!vidyard_SVn4AzM6PEqbuEAzi2ragh_params.hubspot_id) {
        var hubspot_id = fn_getCookie("hubspotutk");
        if (hubspot_id) {
          vidyard_SVn4AzM6PEqbuEAzi2ragh_params.hubspot_id = hubspot_id;
      ...
      if (!vidyard_SVn4AzM6PEqbuEAzi2ragh_params.dreamforce_id) {
        var dreamforce_id = fn_getCookie("vy_dreamforce");
        if (dreamforce_id) {
          dreamforce_id = encodeURIComponent(dreamforce_id);
          vidyard_SVn4AzM6PEqbuEAzi2ragh_params.dreamforce_id = dreamforce_id;
          messages.push({'data': {'type': 'dreamforce', 'value': dreamforce_id}});
        }
      }
      if (!vidyard_SVn4AzM6PEqbuEAzi2ragh_params.marketo_id) {
        var marketo_id = fn_getCookie("_mkto_trk");
        var munchkin_id = '';
        
        if (marketo_id && munchkin_id && marketo_id.toLowerCase().indexOf(munchkin_id) > -1) {
          marketo_id = encodeURIComponent(marketo_id);
          vidyard_SVn4AzM6PEqbuEAzi2ragh_params.marketo_id = marketo_id;
      ...
      if (vidyard_SVn4AzM6PEqbuEAzi2ragh_params.type === "inline" || vidyard_secure_SVn4AzM6PEqbuEAzi2ragh === true) {
        for(var i in messages) {
          messages[i].event = 'associateVisitor';
          messages[i].uuid = 'SVn4AzM6PEqbuEAzi2ragh';
          Vidyard.Helpers.getPlayerIFrame('SVn4AzM6PEqbuEAzi2ragh').contentWindow.postMessage(JSON.stringify(messages[i]), window.location.protocol + '//play.vidyard.com');
        }
      }
    ...
    vidyard_integration_check_SVn4AzM6PEqbuEAzi2ragh = integration_tracking_check;

    // Display the inline or lightbox
    if (vidyard_SVn4AzM6PEqbuEAzi2ragh_params.type === "inline") {
      // Load the iframe
      iframe_container.innerHTML = fn_vidyard_build_iframe_SVn4AzM6PEqbuEAzi2ragh(false, vidyard_player_width_SVn4AzM6PEqbuEAzi2ragh, vidyard_player_height_SVn4AzM6PEqbuEAzi2ragh);

      Vidyard.Helpers.addListener(
        'load',
        'onload',
        function() {
          fn_vidyard_iframe_after_load_SVn4AzM6PEqbuEAzi2ragh();
          integration_tracking_check();
        },
        Vidyard.Helpers.getPlayerIFrame('SVn4AzM6PEqbuEAzi2ragh')
      );

    } else if (vidyard_SVn4AzM6PEqbuEAzi2ragh_params.type === "lightbox") {
      ...
      if (document.readyState === 'complete') {
        if (!vidyard_secure_SVn4AzM6PEqbuEAzi2ragh) {
          integration_tracking_check();
        }
  ...
  Vidyard._players['SVn4AzM6PEqbuEAzi2ragh'] = '';
}());
...
var vyContactCallback = function(data) {
  ...
  vidyard_SVn4AzM6PEqbuEAzi2ragh_params.marketo_lead = marketoData;
  ...
  // or tried again after IFrame loads in fn_vidyard_iframe_after_load_SVn4AzM6PEqbuEAzi2ragh
  playerIframe = Vidyard.Helpers.getPlayerIFrame('SVn4AzM6PEqbuEAzi2ragh');
  if ( playerIframe ) {
    message = {'data': {'type': 'marketo', 'value': marketoData}};
    message.event = 'identifyVisitor';
    message.uuid = 'SVn4AzM6PEqbuEAzi2ragh';
...
(function () {
  function async_load() {
    var contactId = vidyard_SVn4AzM6PEqbuEAzi2ragh_params.marketo_id;
    if (!contactId) {
    ...
    var url = window.location.protocol + '//play.vidyard.com'
        + '/v3/SVn4AzM6PEqbuEAzi2ragh/contact/' + encodeURIComponent(contactId) + '/marketoContact.js';
...
function fn_vidyard_build_iframe_SVn4AzM6PEqbuEAzi2ragh(lightbox, iframe_width, iframe_height) {
  if (lightbox) {
    vidyard_SVn4AzM6PEqbuEAzi2ragh_params.autoplay = true;
  }

  var referring_url = encodeURIComponent(document.referrer);
  if (referring_url) {
    vidyard_SVn4AzM6PEqbuEAzi2ragh_params.referring_url = referring_url;
  }
  ...
  if (document.documentMode === 7) {
    ...
    var ie7RepaintIframe = function() {
      var iframe = Vidyard.Helpers.getPlayerIFrame('SVn4AzM6PEqbuEAzi2ragh');
  ...
  if (vidyard_secure_SVn4AzM6PEqbuEAzi2ragh === true) {
    query_params_string = vidyard_SVn4AzM6PEqbuEAzi2ragh_raw_params;
  } else {
    for (var param in vidyard_SVn4AzM6PEqbuEAzi2ragh_params) {
      var param_value = vidyard_SVn4AzM6PEqbuEAzi2ragh_params[param];
      if (param_value === true) {
        param_value = '1';
      }
  ...
  return '<span id="vidyard_span_SVn4AzM6PEqbuEAzi2ragh" style="display: block; margin: auto; position: relative; width: ' + iframe_width + 'px; height: ' + iframe_height + 'px;"><iframe id="vidyard_iframe_SVn4AzM6PEqbuEAzi2ragh" class="vidyard_iframe" src="//play.vidyard.com/SVn4AzM6PEqbuEAzi2ragh?' + query_params_string + '" width="' + iframe_resizer + '" height="100%" title="Video" aria-label="Video" scrolling=no frameborder=0 allowtransparency="true" allowfullscreen allow="autoplay" style="opacity: 0; background-color: transparent; position: absolute; ' + ios_width_fix + ' right: 0px; top: 0px;"></iframe></span>';
}

// --- If the browser is FF, reload the iframe with the given id
function fn_vidyard_iframe_after_load_SVn4AzM6PEqbuEAzi2ragh() {
  var _theframe = Vidyard.Helpers.getPlayerIFrame('SVn4AzM6PEqbuEAzi2ragh');
  if (_theframe) {
    ...
    if (vidyard_secure_SVn4AzM6PEqbuEAzi2ragh === true && vidyard_SVn4AzM6PEqbuEAzi2ragh_params.type === 'lightbox') {
      vidyard_integration_check_SVn4AzM6PEqbuEAzi2ragh();
    }
    ...
    if (vidyard_SVn4AzM6PEqbuEAzi2ragh_params.marketo_lead) {
      var message = {'data': {'type': 'marketo', 'value': vidyard_SVn4AzM6PEqbuEAzi2ragh_params.marketo_lead }};
      message.event = 'identifyVisitor';
      message.uuid = 'SVn4AzM6PEqbuEAzi2ragh';
      Vidyard.Helpers.getPlayerIFrame('SVn4AzM6PEqbuEAzi2ragh')
...
function fn_vidyard_SVn4AzM6PEqbuEAzi2ragh() {
  var vidyard = {};
    ...
    return {
      show:function(initial_iframe_settings) {
        var showCore = function () {
          ...
          var width_height = this.lock_to_aspect_ratio(iframe_settings.width, iframe_settings.height, (319 + (vidyard_player_width_SVn4AzM6PEqbuEAzi2ragh / 5)));
      ...
      fill_iframe: function(settings_html_url, has_element, use_post_request, should_animate, width, height) {
        if(has_element) {
          if(iframe_settings.image) {
            ...
            this.set_tinner_size(fn_vidyard_build_iframe_SVn4AzM6PEqbuEAzi2ragh(true, width, height), should_animate, width, height);
          } else {
      ...
      resize: function() {
        ...
        var width_height = v_box.lock_to_aspect_ratio(
            vidyard_player_width_SVn4AzM6PEqbuEAzi2ragh,
            vidyard_player_height_SVn4AzM6PEqbuEAzi2ragh,
            319 + (vidyard_player_width_SVn4AzM6PEqbuEAzi2ragh / 5)
        );

        iframe_settings.width = width_height[0];
        iframe_settings.height = width_height[1];

        var vidyard_iframe = Vidyard.Helpers.getPlayerIFrame('SVn4AzM6PEqbuEAzi2ragh');
        var vidyard_span = document.getElementById('vidyard_span_SVn4AzM6PEqbuEAzi2ragh');
      ...
      fade_element: function(element_to_fade, fading_in, target_alpha) {
        ...
        if (old_opacity == target_alpha) {
          ...
          if (fading_in == -1) {
            ...
            Vidyard.Helpers.getPlayerIFrame('SVn4AzM6PEqbuEAzi2ragh').src = '';
  ...
  vidyard.box.show({
    iframe: '#', width: vidyard_player_width_SVn4AzM6PEqbuEAzi2ragh,
    height: vidyard_player_height_SVn4AzM6PEqbuEAzi2ragh,
    animate: !vidyard_html5_SVn4AzM6PEqbuEAzi2ragh,
    fixed: true
  });
}
